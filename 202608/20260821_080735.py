...
IMX = 604                    # x(ω) : 반드시 구간 B 안에 위치
IB0, IB1 = 548, 692          # 구간 B
...
# 표본점
a(f'<circle cx="{OMX}" cy="{OMY}" r="7" fill="{FG}" stroke="{BG}" stroke-width="2"/>')
text(OMX - 14, OMY + 5, 'ω', 15, FG, "600", "end", MATH)

# 확률변수 화살표
a(f'<line x1="{BX+BRX+14}" y1="{CY}" x2="{LX0-18}" y2="{CY}" stroke="{LINE}" '
  f'stroke-width="2" marker-end="url(#ar)"/>')
text((BX + BRX + LX0) / 2, CY - 14, 'x', 16, FG, "600", "middle", MATH, "italic")
text((BX + BRX + LX0) / 2, CY + 26, '확률변수', 14, DIM, "400", "middle")
...
text(IB1 + 4, CY + 28, 'B', 15, FG, "600", "end", MATH, "italic")

# 상 x(ω)
a(f'<circle cx="{IMX}" cy="{CY}" r="7" fill="{FG}" stroke="{BG}" stroke-width="2"/>')
